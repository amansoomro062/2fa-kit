...
[![npm version](https://img.shields.io/npm/v/otpk)](https://www.npmjs.com/package/otpk)
...
- **Encrypted storage built in.** Neither library answers "how do I store the
  secret?" `otpk` ships a vault (PBKDF2 + AES-256-GCM) so secrets are never
...
```ts
import { createVault, generateSecret, buildUri, verifyTotp } from "otpk";

// One vault per process. The master key comes from the environment,
// exactly like a JWT secret.
const vault = await createVault(process.env.OTPK_MASTER_KEY!);
